    ...
    try:
        ...
        while time.time() - start_time < duration:
            ...
            if step <= step_4000:
                ...
                _thread_controlword = 0x000F
                _thread_target_torque = 50  # Set target torque (Reference: C++ 50)
                ...
                try:
                    with _data_exchange_lock:
                        ...
                        if hasattr(master.slaves[0], 'input') and master.slaves[0].input and len(master.slaves[0].input) >= 12:
                            ...
                            # Determine mode indicator for printing
                            mode_indicator = "[UNKNOWN]"
                            if step <= step_10000:
                            ...
                            if sent_controlword is not None:
                                print(f"TX: CW=0x{sent_controlword:04x} TQ={sent_target_torque:4d} MODE={sent_mode:2d} {mode_indicator} | RX: {status_info}, pos={actual_position:8d}, vel={actual_velocity:6d}, torque={actual_torque:4d}, step={step}/{step_10000_display}")
                            else:
                                print(f"TX: CW=0x{_thread_controlword:04x} TQ={_thread_target_torque:4d} MODE={_thread_mode_of_operation:2d} {mode_indicator} | RX: {status_info}, pos={actual_position:8d}, vel={actual_velocity:6d}, torque={actual_torque:4d}, step={step}/{step_10000_display}")
                except Exception as e:
